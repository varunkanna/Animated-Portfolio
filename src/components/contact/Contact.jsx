...
const Contact = () => {
  ...
  const sendEmail = (e) => {
    e.preventDefault();
    emailjs
      .sendForm('service_pz5abbf', 'template_m179rnm', form.current, {
        publicKey: 'dXARczEaBYstf8Yaz',
      })
      .then(
        () => {
          console.log('SUCCESS!');
  ...
  return (
    ...
    >
      <motion.div className="textContainer" variants={varients}>
        <motion.h1 varients={varients}>Let's work together</motion.h1>
        <motion.div className="item">
          <h2>Mail</h2>
          <span>varunganges@gmail.com</span>
        </motion.div>
        <motion.div className="item">
          <h2>Address</h2>
      ...
      <div className="formContainer">
        ...
        <motion.form action="" initial={{opacity: 0}} whileInView={{opacity: 1}} transition={{delay: 4, duration: 1}} ref={form} onSubmit={sendEmail}>
          <input type="text" placeholder="Name" required name="name" onChange={(e) => setName(e.target.value)} value={name} />
          <input type="email" placeholder="Email" required name="email" value={email} onChange={(e) => setEmail(e.target.value)} />
          <textarea placeholder="Message" rows="8" name="message" value={message} onChange={(e) => setMessage(e.target.value)} />
          <button type="submit">Submit</button>
          {error && <span style={{color: "red"}}>Something Wrong</span>}
          {success && <span style={{color: "green"}}>Message send Successfully</span>}
        </motion.form>
      </div>
    </motion.div>
  );
